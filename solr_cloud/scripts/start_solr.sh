...
SOLR_RANGER_HOME=/apps/solr/solr_cloud
SOLR_PORT=6083
SOLR_HOST=`hostname`
...
if [[ -e /etc/hadoop/conf/core-site.xml ]] ; then
        ...
        echo "Hadoop Conf Folder and core-site.xml Missing"
        exit 1
fi
if [[ "$SOLR_ZK_SERVERS" == "" ]] ; then
        echo "No Zookeeper Servers Found"
        exit 1
fi
...
$SOLR_INSTALL_DIR/bin/solr start -cloud -h $SOLR_HOST -p $SOLR_PORT -z $SOLR_ZK_SERVERS/solr_$enviro -d $SOLR_INSTALL_DIR/server -m $SOLR_MEMORY -s $SOLR_RANGER_HOME -Djava.security.auth.login.config=$SOLR_AUTHENTICATION_JAAS_CONF -Dsolr.kerberos.name.rules="`printf "%s\n" $SOLR_AUTHENTICATION_KERBEROS_NAME_RULES`" -Dsolr.kerberos.cookie.domain=$SOLR_AUTHENTICATION_COOKIEDOMAIN  -Dsolr.kerberos.principal=$SOLR_AUTHENTICATION_KERBEROS_PRINCIPAL -Dsolr.kerberos.keytab=$SOLR_AUTHENTICATION_KERBEROS_KEYTAB -DauthenticationPlugin=org.apache.solr.security.KerberosPlugin -Dlog4j.configuration=file://$SOLR_LOG4J_FILEPATH
